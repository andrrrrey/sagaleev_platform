...
| `YOOKASSA_SHOP_ID` / `YOOKASSA_SECRET_KEY` | ЮKassa; для ежемесячного продления магазин должен поддерживать рекуррентные платежи |
| `ANTHROPIC_API_KEY` / `CURATOR_ENABLED` | ответы агента-куратора и его включение |
...
Настройки можно задать через окружение или в `/admin/settings` (значение из БД имеет приоритет). Секреты в код не попадают.
...
- **Оплата:** единая подписка 5 000 ₽ в месяц, `PaymentProvider` + `YookassaProvider` + `MockProvider`, сохранение способа оплаты, автоматическое продление, отмена/возобновление, `/pay`, `/pay/result` (polling) и webhook с идемпотентной активацией `Enrollment`.
- **Экраны:** `/login`, `/register`, `/forgot`, `/reset/[token]`, `/legal/[kind]`, `/onboarding`, `/` (каркас), `/profile` (личные данные, email, пароль и управление подпиской), админ-каркас (обзор, студенты, оплаты, тариф, пользователи), `/api/health`.
...
- **Файлы и видео:** отдельное S3-хранилище не используется; видео показываются напрямую по Kinescope ID, а полные инструкции скиллов хранятся в проекте как `content/skills/*/SKILL.md`.
...
- **Сиды:** маршрут 3 дня со связанными скиллами, базовые маркетинговые карточки и полный каталог из `andrrrrey/agent`.
...
- **Агент-куратор:** включён в единую подписку для каждого активного студента; сборщик контекста **без контактов** (тест на приватность), одиночный вызов Claude Sonnet со structured JSON + ретрай, `CuratorNote` + уведомление; S13 `/profile/curator` (`WeeklyReport`), A10 `/admin/curator` (фичефлаг, ручной запуск, логи), фичефлаг `CURATOR_ENABLED`.
...
3. Выбор единой подписки → mock-оплата мгновенно подтверждается webhook’ом → `/pay/result` показывает «Доступ открыт».
4. `/onboarding` (4 шага) → Главная (каркас).
5. В профиле можно отключить и снова включить автоматическое продление; оплаченный период при отмене сохраняется.
6. Повторный webhook не создаёт дублей (проверяется идемпотентностью по `providerPaymentId`).
7. Прямой запрос закрытого ресурса без подписки → 402/403, поля контента отсутствуют (тесты `tests/access`).
