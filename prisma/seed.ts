  ...
  const routeDays = [
    {
      ...
      summary: 'Разбираемся в трёх простых понятиях, создаём нужные аккаунты и безопасно готовим удалённый компьютер.',
      steps: [
        {
          sort: 0,
          title: '1. Разберись, что именно мы будем собирать',
          body: '<p><strong>Простыми словами.</strong> VPS — это твой компьютер в интернете. Он не выключается вместе с ноутбуком. Hermes — программа-помощник, которую мы поставим на VPS. Telegram-бот — дверь, через которую ты будешь писать помощнику.</p><p><strong>Запомни три пометки:</strong><br>«На сайте» — открой браузер и нажимай кнопки.<br>«На своём компьютере» — открой Terminal на Mac или PowerShell на Windows.<br>«На сервере» — сначала подключись по SSH; слева появится строка вроде agent@server.</p><p><strong>Важно.</strong> Не копируй символы $, # и agent@server из примеров. Никому не отправляй пароль VPS, токен Telegram и ключ модели.</p><p><strong>Готово, если:</strong> ты можешь своими словами объяснить, что VPS — место работы агента, а Telegram — только способ общаться с ним.</p>',
          commands: [{ label: 'Мини-проверка понимания', text: 'Объясни мне совсем простыми словами разницу между: 1) моим компьютером, 2) VPS-сервером, 3) Hermes Agent и 4) Telegram-ботом. После объяснения задай мне четыре коротких вопроса для проверки.', kind: 'prompt' }],
          artifactRequired: true,
          artifactHint: 'одним предложением напиши, где будет работать агент',
          linkedSlug: null,
        },
        {
          sort: 1,
          title: '2. Выбери одну маленькую работу для первого агента',
          body: '<p><strong>Где:</strong> здесь, без сервера и команд.</p><p>Не проси первого агента «вести весь бизнес». Выбери одну повторяющуюся работу с понятным результатом. Хороший пример: сделать черновик ответа клиенту по базе знаний. Плохой пример: самостоятельно продавать, отправлять деньги и удалять данные.</p><p><strong>Сделай по порядку:</strong><br>1. Напиши, что агент получает на входе.<br>2. Напиши, что он должен выдать.<br>3. Укажи человека, который проверяет ответ.<br>4. Засеки, сколько минут эта работа занимает сейчас.<br>5. Реши, какой результат через 30 дней будет успехом.</p><p><strong>Готово, если:</strong> задачу можно проверить одним примером и у неё есть один ответственный человек.</p>',
          commands: [{ label: 'Помощник выбора первой задачи', text: 'Я новичок и хочу запустить первого бизнес-агента. Задай мне по одному простому вопросу о повторяющихся задачах. Затем выбери самую безопасную задачу и заполни карточку: название, что приходит на вход, что должно получиться, кто проверяет, сколько минут занимает сейчас, цель на 30 дней.', kind: 'prompt' }],
          artifactRequired: true,
        ...
        {
          sort: 2,
          title: '3. Запиши, что агенту можно и нельзя',
          body: '<p><strong>Где:</strong> здесь или в обычном документе.</p><p>Агент похож на нового стажёра: сначала он готовит черновики, а важные действия делает только после твоего «да».</p><p><strong>Раздели действия на три коробки:</strong><br>1. «Можно самому» — читать разрешённые материалы, искать информацию, готовить черновик.<br>2. «Только после подтверждения» — отправлять письмо, менять CRM, публиковать пост.<br>3. «Нельзя» — переводить деньги, удалять данные, раскрывать пароли.</p><p>GitHub, VPS, Telegram-бот и аккаунт модели должны принадлежать владельцу бизнеса, а не подрядчику.</p><p><strong>Готово, если:</strong> для каждого возможного действия понятно, в какой оно коробке.</p>',
          commands: [{ label: 'Три коробки безопасности', text: 'Для моей первой задачи составь очень простую таблицу из трёх разделов: «агент может сам», «только после моего подтверждения», «агенту запрещено». Отдельно перечисли, кто владеет VPS, GitHub, Telegram-ботом и аккаунтом модели. Не проси и не показывай пароли или токены.', kind: 'prompt' }],
          artifactRequired: true,
        ...
        {
          sort: 4,
          title: '5. Подключись к VPS — удалённому компьютеру',
          body: '<p><strong>Что подготовить:</strong> IP-адрес VPS, логин root и пароль от хостинга. Числа 203.0.113.10 ниже — пример; замени их на IP своего сервера.</p><p><strong>На Mac:</strong> нажми Command + Space, напиши Terminal и открой программу.<br><strong>На Windows:</strong> открой Пуск, напиши PowerShell и открой его.</p><p>Вставь команду подключения и нажми Enter. При первом входе сравни fingerprint с панелью хостинга, затем напиши yes. Введи пароль: символы на экране не появляются — это нормально.</p><p><strong>Готово, если:</strong> слева появилась строка, похожая на root@server:~#.</p><p><strong>Частые ошибки:</strong><br>Permission denied — неверный логин или пароль.<br>Connection timed out — неверный IP, сервер выключен или порт закрыт.<br>Connection refused — SSH на сервере не запущен. Проверь данные в панели хостинга, а не повторяй пароль много раз.</p>',
          commands: [{ label: 'НА СВОЁМ КОМПЬЮТЕРЕ — замени IP', text: 'ssh root@203.0.113.10', kind: 'command' }],
          artifactRequired: true,
          artifactHint: 'напиши «вижу строку root@...», но не копируй пароль',
          linkedSlug: null,
        },
        {
          sort: 5,
          title: '6. Создай безопасного пользователя и обнови сервер',
          body: '<p><strong>Где:</strong> на сервере, где слева написано root@...</p><p>Root — главный ключ от всего сервера. Агент не должен постоянно работать с таким ключом, поэтому создаём отдельного пользователя agent.</p><p><strong>Сначала:</strong> выполни первые четыре команды по одной. Команда adduser попросит придумать пароль; сохрани его. Остальные вопросы можно пропустить клавишей Enter. После su - agent команда whoami должна ответить agent.</p><p><strong>Потом:</strong> установи обновления. Команда sudo может попросить пароль пользователя agent. Перед firewall обязательно разреши OpenSSH, иначе можно закрыть себе вход.</p><p><strong>Не делай пока:</strong> не отключай root и вход по паролю. Сначала в следующем шаге второго дня проверим ключ.</p><p><strong>Готово, если:</strong> whoami отвечает agent, обновления установлены, а ufw status показывает Status: active и правило OpenSSH.</p>',
          commands: [
            { label: 'НА СЕРВЕРЕ ПОД ROOT — создать agent', text: 'adduser agent\nusermod -aG sudo agent\nsu - agent\nwhoami', kind: 'command' },
            { label: 'НА СЕРВЕРЕ ПОД AGENT — обновить и защитить', text: 'sudo apt update\nsudo apt upgrade -y\nsudo ufw allow OpenSSH\nsudo ufw enable\nsudo ufw status verbose', kind: 'command' },
          ],
          artifactRequired: true,
          artifactHint: 'ответ whoami и строки Status/OpenSSH без личных данных',
          linkedSlug: null,
        },
      ],
    },
    {
      ...
      steps: [
        ...
        {
          sort: 3,
          title: '4. Подключи «мозг» агента — модель',
          body: '<p><strong>Простыми словами.</strong> Hermes — тело и инструменты, а модель Claude или Codex — мозг. Без модели агент не сможет отвечать.</p><p><strong>Где вводить ключ:</strong> только на сервере внутри мастера hermes setup, когда он сам попросит API key. Не вставляй ключ на эту платформу, в GitHub, AGENTS.md или поле отчёта.</p><p><strong>Если используешь Codex:</strong> выбери официальный вход через OAuth, если мастер его предлагает, и перейди по ссылке авторизации.<br><strong>Если используешь Claude/API:</strong> создай ключ в кабинете провайдера, задай лимит расходов и вставь ключ прямо в скрытое поле мастера.</p><p>Аккаунт модели и оплата должны принадлежать владельцу бизнеса. После настройки выйди в диалог и отправь одну тестовую фразу.</p><p><strong>Готово, если:</strong> агент ответил на тест, а ты записал название модели и месячный лимит — без ключа.</p>',
          commands: [{ label: 'НА СЕРВЕРЕ — мастер и проверка', text: 'hermes setup\nhermes model\nhermes doctor\nhermes\n# В открывшемся диалоге напиши:\nОтветь одной строкой: модель подключена и диалог работает.', kind: 'command' }],
          artifactRequired: true,
        ...
        },
        {
          ...
          commands: [
            ...
            { label: 'ТАБЛИЦА ДЛЯ 10 УЧЕБНЫХ ПРИМЕРОВ', text: 'Номер | Вопрос | Какой ответ ожидали | Что ответил агент | Правильно? | Нужно исправить | Опасное действие без разрешения?', kind: 'prompt' },
          ],
          artifactRequired: true,
      ...
      steps: [
        ...
        },
        {
          ...
          commands: [
            ...
            { label: 'ЧЕК-ЛИСТ ЭКЗАМЕНА', text: 'Проведи меня по финальной проверке агента. Задавай по одному пункту: личность, знания и источник, честное «не знаю», подтверждение внешнего действия, защита секретов, Telegram allowlist, запуск после reboot, лимит расходов. В конце покажи таблицу Pass/Fail и простое исправление для каждого Fail.', kind: 'prompt' },
          ],
          artifactRequired: true,
        ...
        },
        {
          ...
          commands: [{ label: 'КАРТОЧКА ПЕРВОГО СЦЕНАРИЯ', text: 'Помоги запустить первую задачу агента в безопасном режиме черновика. Заполни карточку простыми словами: когда запускается, что читает, что создаёт, кто проверяет, что агенту запрещено отправлять самому, лимит расходов, журнал, команда остановки и показатели через 7 дней.', kind: 'prompt' }],
          artifactRequired: true,
