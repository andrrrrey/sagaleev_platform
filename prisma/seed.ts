...
async function main() {
  ...
  const routeDays = [
    {
      dayNumber: 1, title: 'Подготовка и безопасный VPS', artifact: 'Готовый VPS, границы агента и заполненная бизнес-память',
      summary: 'Выбираем один бизнес-процесс, фиксируем права и границы, готовим Ubuntu VPS и приватный репозиторий.',
      steps: [
        {
          sort: 0, title: '1. Выбери один процесс и измеримый результат',
          body: '<p>Не начинай с «агента, который делает всё». Выбери один узкий процесс: например, черновик ответа, ежедневная сводка, квалификация лида или поиск по базе знаний.</p><p><strong>Зафиксируй:</strong> владелец процесса, текущее время/стоимость, цель на 30 дней и одну метрику успеха.</p>',
          commands: [{ label: 'Карточка процесса', text: 'Помоги выбрать один узкий бизнес-процесс для первого AI-агента. Задай мне вопросы о частоте, трудозатратах, риске и измеримом результате. В конце дай одну карточку: вход, выход, владелец, baseline, цель на 30 дней.', kind: 'prompt' }],
          artifactRequired: true, artifactHint: 'карточка процесса и baseline', linkedSlug: 'safe-autonomous-work',
        },
        {
          sort: 1, title: '2. Запиши границы, данные и владение',
          body: '<p>Раздели действия на три класса: агент делает сам; делает после подтверждения; не делает никогда. Отдельно перечисли данные, срок хранения, разрешённые системы и того, кто одобряет новые интеграции.</p><p>VPS, Telegram-бот, GitHub и аккаунт модели должны принадлежать клиенту. Не пересылай токены в обычном чате.</p>',
          commands: [{ label: 'Матрица прав', text: 'Для выбранного процесса составь таблицу: «действие / самостоятельно / после подтверждения / запрещено». Отдельно составь реестр данных и владельцев аккаунтов. Не включай секреты.', kind: 'prompt' }],
          artifactRequired: true, artifactHint: 'матрица прав и реестр владения', linkedSlug: 'safe-autonomous-work',
        },
        {
          sort: 2, title: '3. Создай отдельного пользователя agent и SSH-ключ',
          body: '<p>Подключись к Ubuntu VPS и создай отдельного пользователя. Проверь вход по ключу во второй SSH-сессии до любых изменений root/парольного входа.</p><p><strong>Важно:</strong> постоянный агент не должен работать от root.</p>',
          commands: [
            { label: 'Пользователь на VPS', text: 'adduser agent\nusermod -aG sudo agent\nsu - agent\nwhoami', kind: 'command' },
            { label: 'SSH-ключ на своём компьютере', text: 'ssh-keygen -t ed25519 -C "client-agent-access"', kind: 'command' },
          ],
          artifactRequired: true, artifactHint: 'успешный вход agent@IP по SSH-ключу', linkedSlug: null,
        },
        {
          sort: 3, title: '4. Обнови Ubuntu и включи firewall',
          body: '<p>Проверь Ubuntu LTS, установи security-обновления, разреши SSH и только затем включи firewall. Telegram long polling не требует открытого входящего порта.</p><p>Проверь, какие порты слушаются, и не открывай лишние.</p>',
          commands: [{ label: 'Подготовка Ubuntu', text: 'cat /etc/os-release\nsudo apt update\nsudo apt upgrade -y\nsudo ufw allow OpenSSH\nsudo ufw enable\nsudo ufw status verbose\nss -lntup', kind: 'command' }],
          artifactRequired: true, artifactHint: 'версия Ubuntu, UFW active и список открытых портов', linkedSlug: null,
        },
        {
          sort: 4, title: '5. Подключи приватный GitHub и заполни знания',
          body: '<p>Создай на VPS отдельный deploy key и добавь в GitHub только публичную часть. Склонируй репозиторий по SSH, а не по HTTPS-паролю.</p><p>Заполни COMPANY, ICP, OFFERS, BRAND-VOICE, METRICS и POLICIES. В Git не должно быть .env, токенов, паролей и клиентских выгрузок. <a href="https://github.com/andrrrrey/agent" target="_blank" rel="noreferrer">Открыть исходную инструкцию</a>.</p>',
          commands: [{ label: 'Deploy key и клонирование', text: 'ssh-keygen -t ed25519 -C "client-agent-deploy" -f ~/.ssh/client_agent_deploy\ncat ~/.ssh/client_agent_deploy.pub\n# После добавления deploy key в GitHub:\ngit clone git@github.com:<OWNER>/<PRIVATE_REPO>.git ~/client-agent-starter', kind: 'command' }],
          artifactRequired: true, artifactHint: 'ссылка на приватный репозиторий и чек-лист заполненных knowledge-файлов', linkedSlug: 'agent-pamyat',
        },
      ],
    },
    {
      dayNumber: 2, title: 'Hermes, личность и первый workflow', artifact: 'Агент с правилами, бизнес-памятью, скиллами и пройденным тестом',
      summary: 'Устанавливаем Hermes из официального источника, подключаем клиентскую модель, правила, память и узкий набор скиллов.',
      steps: [
        {
          sort: 0, title: '1. Установи Hermes Agent из официального источника',
          body: '<p>Сначала открой официальный install-скрипт в режиме просмотра. Если адрес или содержимое не совпадают с официальной документацией, остановись. После установки проверь CLI и doctor.</p>',
          commands: [{ label: 'Установка и проверка', text: 'curl -fsSL https://hermes-agent.nousresearch.com/install.sh | less\n# После просмотра:\ncurl -fsSL https://hermes-agent.nousresearch.com/install.sh | bash\nsource ~/.bashrc\nhermes --help\nhermes doctor', kind: 'command' }],
          artifactRequired: true, artifactHint: 'версия Hermes и результат hermes doctor', linkedSlug: null,
        },
        {
          sort: 1, title: '2. Подключи модель и проверь обычный диалог',
          body: '<p>Выбери поддерживаемый Hermes способ: клиентский API-аккаунт с лимитом расходов или официальную подписочную авторизацию, если она подходит по условиям и лимитам. Не копируй личную авторизацию установщика.</p><p>Зафиксируй провайдера, модель, способ авторизации, владельца биллинга и лимит — без самого секрета.</p>',
          commands: [{ label: 'Модель и smoke test', text: 'hermes setup\nhermes model\nhermes doctor\nhermes\n# Запрос: «Ответь одной строкой: модель подключена и диалог работает».', kind: 'command' }],
          artifactRequired: true, artifactHint: 'провайдер/модель/лимит и успешный тестовый ответ', linkedSlug: null,
        },
        {
          sort: 2, title: '3. Подключи SOUL, AGENTS.md и бизнес-память',
          body: '<p>Скопируй заполненные файлы в профиль Hermes и рабочую папку. Проверь имя, компанию, границы, источники и запрет на внешние действия без подтверждения.</p>',
          commands: [{ label: 'Правила и knowledge', text: 'cp ~/client-agent-starter/templates/SOUL.md ~/.hermes/SOUL.md\nmkdir -p ~/client-agent-workspace\ncp ~/client-agent-starter/AGENTS.md ~/client-agent-workspace/AGENTS.md\ncp -R ~/client-agent-starter/knowledge ~/client-agent-workspace/knowledge\nhermes config set terminal.cwd /home/agent/client-agent-workspace', kind: 'command' }, { label: 'Контрольные вопросы', text: 'Как тебя зовут и какую компанию ты представляешь?\nКакие действия ты можешь делать сам?\nЧто требует моего подтверждения?\nНазови продукты компании и укажи источник.', kind: 'prompt' }],
          artifactRequired: true, artifactHint: 'файлы SOUL/AGENTS/knowledge и корректные контрольные ответы', linkedSlug: 'agent-pamyat',
        },
        {
          sort: 3, title: '4. Установи только стартовый набор скиллов',
          body: '<p>Большой каталог — витрина возможностей, а не команда установить всё. Для первого workflow выбери 3–7 узких скиллов. Скилл описывает порядок работы, но не даёт новых API-прав.</p>',
          commands: [{ label: 'Безопасный стартовый набор', text: 'mkdir -p ~/.hermes/skills/business\ncp -R ~/client-agent-starter/skills/knowledge-base-answer ~/.hermes/skills/business/\ncp -R ~/client-agent-starter/skills/customer-reply-draft ~/.hermes/skills/business/\ncp -R ~/client-agent-starter/skills/business-daily-brief ~/.hermes/skills/business/\ncp -R ~/client-agent-starter/skills/safe-autonomous-work ~/.hermes/skills/business/\nhermes skills list', kind: 'command' }],
          artifactRequired: true, artifactHint: 'вывод hermes skills list и обоснование выбранных 3–7 скиллов', linkedSlug: 'knowledge-base-answer',
        },
        {
          sort: 4, title: '5. Прогони первый workflow на контрольном наборе',
          body: '<p>Подготовь 10–30 обезличенных примеров: обычные, неполные, противоречивые, запрещённое действие, prompt injection и повторный запуск. Измерь точность, ручные правки, время, стоимость и опасные действия без подтверждения (должно быть 0).</p><p>Не переходи к автоматическим действиям, пока запрещённые сценарии, откат, журнал и лимиты не пройдены.</p>',
          commands: [{ label: 'Формат eval', text: 'Сценарий | Ожидаемый результат | Факт | Источники | Нужные правки | Опасное действие | Время | Стоимость | Pass/Fail', kind: 'prompt' }],
          artifactRequired: true, artifactHint: 'таблица eval с 10–30 примерами и релизным порогом', linkedSlug: 'safe-autonomous-work',
        },
      ],
    },
    {
      dayNumber: 3, title: 'Telegram 24/7, автозапуск и приёмка', artifact: 'Принятый владельцем агент 24/7 с allowlist, автозапуском, backup и планом остановки',
      summary: 'Подключаем отдельного Telegram-бота, запускаем gateway как systemd-службу, проверяем после reboot и передаём владельцу.',
      steps: [
        {
          sort: 0, title: '1. Создай отдельного Telegram-бота и узнай numeric ID',
          body: '<p>Владелец создаёт нового бота у @BotFather и сохраняет токен в менеджере паролей. Username вида @ivan не заменяет числовой Telegram ID.</p><p>Токен не нужно вставлять в платформу, GitHub, артефакт или скриншот. Если он утёк, сразу отзови его в BotFather.</p>',
          commands: [], artifactRequired: true, artifactHint: 'имя бота и числовой Telegram ID без токена', linkedSlug: 'agent-telegram',
        },
        {
          sort: 1, title: '2. Настрой Telegram gateway только с allowlist',
          body: '<p>В мастере введи токен непосредственно на VPS и добавь в allowlist только согласованные numeric ID. Не включай open access.</p><p>Сначала запусти gateway в foreground, проверь владельца и постороннего пользователя, затем останови Ctrl+C. Один токен нельзя опрашивать двумя копиями gateway.</p>',
          commands: [{ label: 'Gateway в тестовом режиме', text: 'hermes gateway setup\nhermes gateway run\n# После теста остановить Ctrl+C', kind: 'command' }],
          artifactRequired: true, artifactHint: 'успешный ответ владельцу и отказ/отсутствие доступа для ID вне allowlist', linkedSlug: 'agent-telegram',
        },
        {
          sort: 2, title: '3. Запусти gateway как системную службу',
          body: '<p>Установи официальную systemd-службу и проверь статус и свежие логи. Перед запуском убедись, что foreground-копия остановлена.</p><p>Не публикуй полные логи: в них могут быть внутренние данные.</p>',
          commands: [{ label: 'Systemd-служба', text: 'sudo hermes gateway install --system\nsudo hermes gateway start --system\nsudo hermes gateway status --system\njournalctl -u hermes-gateway -n 100 --no-pager', kind: 'command' }],
          artifactRequired: true, artifactHint: 'статус active (running) и обезличенный фрагмент лога без ошибок', linkedSlug: null,
        },
        {
          sort: 3, title: '4. Проверь автозапуск после перезагрузки VPS',
          body: '<p>Предупреди о короткой недоступности, перезагрузи VPS, переподключись и проверь status, deep status, doctor и реальный Telegram-ответ. Агент должен работать при закрытом ноутбуке.</p>',
          commands: [{ label: 'Перезагрузка и smoke test', text: 'sudo reboot\n# Через 1–2 минуты войти снова:\nsudo hermes gateway status --system\nhermes gateway status --deep --system\nhermes doctor', kind: 'command' }],
          artifactRequired: true, artifactHint: 'статус после reboot и свежий ответ бота', linkedSlug: null,
        },
        {
          sort: 4, title: '5. Проведи приёмку и подключай автоматизации поэтапно',
          body: '<p>Проверь: имя/компанию, allowlist, границы, ссылки на источники, запрещённое действие, автозапуск, лимит расходов, backup и восстановление. Передай владельцу команды status/restart/stop/start/doctor, схему расходов и дату следующего обновления.</p><p>Первую автоматизацию начни с read-only сводки или черновика. Для каждого API/MCP нужна карточка: владелец, цель, данные, действия, минимальные права, лимит, журнал и способ отключения.</p>',
          commands: [{ label: 'Команды владельца', text: 'sudo hermes gateway status --system\nsudo hermes gateway restart --system\nsudo hermes gateway stop --system\nsudo hermes gateway start --system\nhermes gateway status --deep --system\nhermes doctor', kind: 'command' }, { label: 'Финальная проверка', text: 'Проведи приёмку агента: после reboot, личность, allowlist, границы, источники, запрещённое действие, секреты, лимит расходов, backup/восстановление и команды владельца. Верни Pass/Fail по каждому пункту.', kind: 'prompt' }],
          artifactRequired: true, artifactHint: 'подписанный чек-лист приёмки, backup и карточка первой read-only автоматизации', linkedSlug: 'business-daily-brief',
        },
      ],
    },
  ];
