      ...
      steps: [
        {
          ...
          body: '<p><strong>Зачем этот шаг.</strong> Дальше ты будешь переходить между четырьмя местами. Большинство ошибок у новичка происходит не из-за сложной команды, а потому что правильный текст вставлен не в то окно.</p><p><strong>Окно 1 — эта платформа.</strong> Здесь ты читаешь урок, нажимаешь «Скопировать», отмечаешь «Сделал» и сохраняешь безопасный итог. Команды здесь не выполняются.</p><p><strong>Окно 2 — ChatGPT или Codex.</strong> Это помощник во время установки. Открой <a href="https://chatgpt.com/" target="_blank" rel="noreferrer">chatgpt.com</a>, войди и создай новый чат с названием «Мой первый агент». Все оранжевые блоки «Текст для ИИ» вставляй новым сообщением именно туда. Дождись ответа, прочитай его и только затем возвращайся сюда. Codex можно использовать вместо ChatGPT, но это необязательно.</p><p><strong>Нужна ли платная подписка прямо сейчас?</strong> Для первых упражнений достаточно доступного тебе аккаунта ChatGPT. Codex сейчас доступен в планах ChatGPT, но лимиты отличаются. API-ключ для шагов 1–3 не нужен. Отдельный способ оплаты модели для постоянно работающего Hermes выберем только во второй день.</p><p><strong>Окно 3 — Telegram.</strong> Здесь создаём бота и потом общаемся с готовым агентом.</p><p><strong>Окно 4 — Terminal на Mac или PowerShell на Windows.</strong> Это окно для команд. Пока ты не подключился к VPS, команда выполняется на твоём компьютере. После команды ssh строка слева станет похожа на root@server или agent@server — с этого момента команда выполняется на сервере.</p><p><strong>Какие аккаунты понадобятся за три дня:</strong><br>1. ChatGPT — для подсказок и, если выберешь этот путь, авторизации модели.<br>2. Telegram — для создания бота.<br>3. GitHub — для приватной копии правил и знаний агента.<br>4. VPS-хостинг — для круглосуточного сервера.<br>5. Аккаунт провайдера модели — может совпасть с ChatGPT или быть отдельным API-аккаунтом.</p><p><strong>Что подготовить сейчас:</strong><br>1. Открой эту платформу в первой вкладке.<br>2. Открой ChatGPT или Codex во второй вкладке и создай новый чат.<br>3. Открой Telegram.<br>4. Mac: Command + Space → Terminal. Windows: Пуск → PowerShell.<br>5. Установи менеджер паролей или подготовь защищённое хранилище. Обычная заметка и чат не подходят.<br>6. Проверь, что можешь войти в GitHub; если аккаунта нет, зарегистрируй его на github.com.</p><p><strong>Важно.</strong> Символы $, # и текст root@server из примеров копировать не надо. Пароль VPS, токен Telegram и API-ключ нельзя вставлять ни в ChatGPT/Codex, ни в поле отчёта на платформе.</p><p><strong>Готово, если:</strong> открыты четыре окна, есть менеджер паролей и доступ к GitHub.</p>',
          commands: [{ label: 'CHATGPT ИЛИ CODEX — вставь в созданный чат', text: 'Я совсем не технический пользователь. Во время этого курса объясняй мне по одному действию за раз и всегда начинай ответ словами «Открой такое-то окно». Не проси меня присылать пароли, токены или API-ключи. Сейчас коротко проверь, понимаю ли я разницу между платформой курса, чатом ChatGPT/Codex, Telegram и терминалом.', kind: 'prompt' }],
        ...
        },
        {
          ...
          body: '<p><strong>Где выполнять:</strong> в окне ChatGPT или Codex из прошлого шага. На сервер пока заходить не надо.</p><p><strong>Что означает «вход».</strong> Это то, что человек передаёт агенту: например, вопрос клиента и утверждённая база знаний. <strong>«Результат»</strong> — то, что агент возвращает: например, черновик ответа. Первый агент не должен «вести весь бизнес».</p><p><strong>Сделай точно по порядку:</strong><br>1. Нажми «Скопировать» под блоком ниже.<br>2. Перейди в чат «Мой первый агент».<br>3. Вставь текст в строку сообщения и нажми Enter.<br>4. ИИ будет задавать вопросы. Отвечай обычными словами, по одному сообщению. Секреты и клиентские персональные данные не отправляй.<br>5. Когда ИИ покажет итоговую карточку, проверь её сам.<br>6. Скопируй только итоговую карточку и вставь в поле «Что сохранить после шага» на этой платформе.</p><p><strong>Хороший первый сценарий:</strong> подготовить черновик ответа клиенту, сделать ежедневную сводку или найти ответ в базе знаний. <strong>Плохой:</strong> самостоятельно отправлять деньги, удалять данные, публиковать рекламу или обещать цену клиенту.</p><p><strong>Готово, если:</strong> в карточке есть один вход, один результат, один проверяющий человек и понятное время.</p>',
          commands: [{ label: 'CHATGPT ИЛИ CODEX — новое сообщение в том же чате', text: 'Помоги выбрать только одну первую задачу для бизнес-агента. Я новичок. Задавай мне по одному короткому вопросу и жди моего ответа. Выбирай среди повторяющихся задач, где агент сначала готовит только черновик. Не предлагай отправку денег, удаление данных, самостоятельную публикацию или сообщение клиенту без проверки человека. В конце выдай одну карточку: 1) название задачи, 2) что человек передаёт агенту на входе, 3) что агент возвращает, 4) кто проверяет результат, 5) сколько минут задача занимает сейчас, 6) какой результат через 30 дней будет успехом.', kind: 'prompt' }],
          artifactRequired: true,
