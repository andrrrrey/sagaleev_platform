...
> **Правило №1.** Стиль берём из эталона буквально. Это светлая «бумажная» техническая эстетика (тёплый бежевый `#f6f5ef`) в тёмной внешней раме `#111110`. **Светлая тема — канонический вид 1:1 с эталоном и остаётся по умолчанию.** Gold-акценты из КП **не используются**. Акцент один: оранжево-красный `#d95321`.
>
> **Исключение (согласовано с заказчиком, 2026):** добавлена *опциональная* тёмная тема — «инверсия бумаги» из тех же токенов (не gold, тот же акцент `#d95321`). Она включается пользователем и следует `prefers-color-scheme`; светлая остаётся дефолтной. Реализация и палитра — в §11. Это не отменяет сверку светлого вида 1:1 с эталоном.
>
...
## 11. Тёмная тема (опциональная, светлая по умолчанию)

Согласованное расширение: тёмный режим как «инверсия бумаги» на тех же токенах. Светлая тема — дефолт и канон 1:1 с эталоном; тёмная включается пользователем.

### 11.1 Архитектура токенов
Цвета вынесены в CSS-переменные (RGB-тройки) и подключены в Tailwind как `rgb(var(--c-*) / <alpha-value>)`. Это сохраняет модификаторы прозрачности (`bg-paper/40`, `border-line/70`, `text-accent/30`) и переключает тему без правки классов в компонентах.

- Светлые значения `--c-*` в `:root` **точно равны** hex из эталона (см. §1.1) → светлый вид не изменился.
- Тёмные значения — в `:root[data-theme="dark"]` и в `@media (prefers-color-scheme: dark)` для `:root:not([data-theme="light"]):not([data-theme="dark"])`.
- Файлы: `src/styles/globals.css` (переменные + декоративные наложения через `--c-line-solid`, `--hatch-*`, `--grid-rgb`), `tailwind.config.ts` (маппинг токенов).

Семантические классы вместо «сырых» hex/zinc: `bg-paper|paper-panel|paper-tint|paper-hover`, `bg-surface`, `border-line`, `bg-ink`/`text-ink-hover`, `text/bg/border-accent`, текстовая шкала `t900…t300` (в светлой теме = точные `zinc-900…300`).

### 11.2 Тёмная палитра
Акцент `#d95321` не меняется. Ключевые токены (тёмная): подложка `page` → почти чёрный; контейнер `paper` → тёмный уголь; панели `paper-panel`/`surface` — чуть светлее; `line` — приглушённая граница; `ink` (первичная кнопка) инвертируется в светлую плашку с тёмным текстом; текстовая шкала `t900…t300` — светлая рампа на тёмном. Точные значения — в `globals.css`.

### 11.3 Переключатель и анти-мигание
- `src/components/theme/theme-script.tsx` — инлайн-скрипт в `<head>`, применяет сохранённую тему до первой отрисовки (без FOUC).
- `src/components/theme/ThemeToggle.tsx` — цикл «системная → светлая → тёмная», хранит выбор в `localStorage('theme')`, иконки `sun-2` / `moon` / `monitor-smartphone`. Встроен в `Nav`, `PublicNav`, `AccountNav`.
- Тёмная тема — только токены (никаких новых цветов), поэтому §10-чеклист и «нет цветов кроме токенов» соблюдаются в обоих режимах.
